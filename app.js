// Acry Finance Hub - Main Application Logic
// Sample data
...
function initializeApp() {
    ...
    document.querySelectorAll('.menu-item').forEach(item => {
        item.addEventListener('click', function(e) {
            showSection(item.getAttribute('data-section'), e);
        });
    });
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            showMarketTab(btn.getAttribute('data-tab'), e);
        });
    });
}
...
function updateDashboard() {
    ...
    const portfolioValue = transactions.filter(t => t.category === 'Investment').reduce((sum, t) => t.type === 'income' ? sum + t.amount : sum - t.amount, 0);
...
function updateFinancialHealth(monthlyIncome, monthlyExpenses, totalBalance) {
    ...
    let status = '一般';
    let details = '';
    if (monthlyIncome > 0) score += ((monthlyIncome - monthlyExpenses) / monthlyIncome) * 30;
    if (totalBalance > 10000) score += 20; else if (totalBalance > 0) score += 10; else score -= 20;
    if (monthlyExpenses > monthlyIncome) score -= 15;
    if (score >= 80) { status = '優秀'; details = '您的財務狀況非常健康！繼續保持良好的理財習慣。'; }
    else if (score >= 60) { status = '良好'; details = '您的財務狀況良好，建議增加儲蓄比例。'; }
    else if (score >= 40) { status = '一般'; details = '需要注意支出控制，建議制定預算計劃。'; }
    else { status = '需改善'; details = '財務狀況需要改善，請減少非必要支出並增加收入。'; }
...
function populateCategories() {
    ...
    categorySelects.forEach(selectId => {
        ...
        if (selectId === 'filter-category') select.innerHTML = '<option value="">所有類別</option>';
...
function initializeCharts() { createExpenseChart(); createTrendChart(); }
...
function createTrendChart() {
    ...
    trendChart = new Chart(ctx, {
        ...
        options: {
            ...
            maintainAspectRatio: false,
            scales: {
                y: {beginAtZero: true, grid: {color: '#374151'}, ticks: {color: '#e5e7eb'}},
...
function getTrendData() {
    ...
    last6Months.forEach(({ year, monthIndex }) => {
        const monthIncome = transactions.filter(t => {
            const transactionDate = new Date(t.date);
            return t.type === 'income' && transactionDate.getFullYear() === year && transactionDate.getMonth() === monthIndex;
        }).reduce((sum, t) => sum + t.amount, 0);
        const monthExpenses = transactions.filter(t => {
            const transactionDate = new Date(t.date);
            return t.type === 'expense' && transactionDate.getFullYear() === year && transactionDate.getMonth() === monthIndex;
        }).reduce((sum, t) => sum + t.amount, 0);
...
function calculateBankruptcyPrediction() {
    ...
    const totalBalance = transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0) -
        transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    let riskLevel = 'low', riskText = '低風險', recommendation = '您的財務狀況健康，繼續保持良好的理財習慣。';
    const expenseRatio = monthlyIncome > 0 ? monthlyExpenses / monthlyIncome : 1;
    if (expenseRatio > 0.9 || totalBalance < 0) {
        riskLevel = 'high'; riskText = '高風險';
        recommendation = '警告：支出過高或負債嚴重，建議立即削減非必要支出並增加收入來源。';
    } else if (expenseRatio > 0.7 || totalBalance < 1000) {
        riskLevel = 'medium'; riskText = '中風險';
...
function showMarketTab(tabName, event) {
...
function updateForexConverter() {
    ...
    if (fromCurrency !== toCurrency) {
        ...
        if (directRate) result = amount * directRate.rate;
        else if (reverseRate) result = amount / reverseRate.rate;
        else if (fromCurrency === 'HKD' && toToHKD) result = amount / toToHKD.rate;
        else if (toCurrency === 'HKD' && fromToHKD) result = amount * fromToHKD.rate;
        else if (fromToHKD && toToHKD) result = amount * fromToHKD.rate / toToHKD.rate;
    }
...
function shareReport() {
    ...
    if (navigator.share) {
        ...
        showNotification('分享功能不支援此瀏覽器！', 'error');
    }
}
...
style.textContent = `
    ...
    .notification.error {
        background: var(--color-error, #DB4545);
    }
`;
